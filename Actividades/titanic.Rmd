...
# Actividad 1: Redes neuronales con caret

## 1. Aprendizaje de un modelo de clasificación utilizando 'Random Forest' (lo llamaremos modelo_rf1) [ver línea 437 y siguientes]

```{r act1}
rfCtrl <- trainControl(classProbs = TRUE, summaryFunction = twoClassSummary)
# Train model without cross-validation
modelo_rf1 <- train(Survived ~ ., data = train, method = "rf", metric = "ROC", , trControl = rpartCtrl)
# Get prediction validation probabilities
predictionValidationProb <- predict(modelo_rf1, val, type = "prob")
# Get prediction validation
predictionValidation <- predict(modelo_rf1, val)
...
acc <- confusionMatrix(predictionValidation, val$Survived)
acc$overall["Accuracy"]
```

Y a continuación con el conjunto de test:

```{r act1-2}
# Get prediction validation
predictionTest <- predict(modelo_rf1, test)
predictionTest
```
...
rpartCtrl <- trainControl(verboseIter = TRUE, classProbs = TRUE, summaryFunction = twoClassSummary, method = "cv", number = 10)
# Train model
modelo_rf2 <- train(Survived ~ ., data = train, method = "rf", metric = "ROC", trControl = rpartCtrl)
# Get prediction validation probabilities
predictionValidationProb <- predict(modelo_rf2, val, type = "prob")
# Get prediction validation
predictionValidation <- predict(modelo_rf2, val)

# Get AUC by ROC curve
auc2 <- roc(val$Survived, predictionValidationProb[["Yes"]], levels = unique(val[["Survived"]]))
roc_validation2 <- plot.roc(auc2, ylim=c(0,1), type = "S" , print.thres = T, main=paste('Validation AUC:', round(auc2$auc[[1]], 2)))

# Get accuracy by confusion matrix
acc2 <- confusionMatrix(predictionValidation, val$Survived)
acc2$overall["Accuracy"]
```
...
```{r act2-3}
# Get prediction validation
predictionTest <- predict(modelo_rf2, test)
predictionTest
```

## 3. Comparación y selección del mejor modelo en modelo_rf1, modelo_rf2 términos de precisión y AUC (modelo_rf)

Como se puede observar el modelo _modelo_rf2_ posee un mayor valor AUC de media siendo este 0.86 frente al 0.81 del _modelo_rf1_. Por otro lado, en términos de precisión se puede apreciar que el 0.7932331 de precisión que posee el modelo _modelo_rf1_ es claramente inferior al 0.8305085 del modelo _modelo_rf2_, por lo que se concluye con que el modelo _modelo_rf2_ es el claro ganador de esta comparación.
...
```{r act4}
# Train model
modelo_rna <- train(Survived ~ ., data = train, method = "nnet", metric = "ROC", trControl = rpartCtrl)
# Get prediction validation probabilities
...
roc_validation_rna <- plot.roc(auc_rna, ylim=c(0,1), type = "S" , print.thres = T, main=paste('Validation AUC:', round(auc$auc[[1]], 2)))
...
```{r act4-2}
# Get prediction validation
predictionTest <- predict(modelo_rna, test)
predictionTest
```

## 5. Envío de solución a Kaggle con modelo_rna

Y exportamos los datos:

```{r act5}
prediction_table <- 
  select(test, PassengerId) %>%
  mutate(Survived = 
           ifelse(predictionTest=='Yes', 1, 0)) 
   
write_csv(prediction_table, "modelo_rna.csv")
```

![Resultados predicción rna mejorado](./modelo_rna.png)
...
```{r act6}
# Train model extracted from: https://stackoverflow.com/questions/42417948/how-to-use-size-and-decay-in-nnet
...
roc_validation_rna_mejorado <- plot.roc(auc_rna_mejorado, ylim=c(0,1), type = "S" , print.thres = T, main=paste('Validation AUC:', round(auc_rna_mejorado$auc[[1]], 2)))
...
```{r act6-2}
# Get prediction validation
predictionTest <- predict(modelo_rna_mejorado, test)
predictionTest
```
...
![Resultados predicción rna mejorado](./modelo_rna_mejorado.png)

## 8. Comparación de modelo_rna_mejorado con modelo_rf

Como se puede observar, los resultados son prácticamente idénticos, ya que se obtiene sobre el conjunto de validación la misma precisión y el mismo valor AUC, por lo que cabría concluir que los modelos construidos toman decisiones similares y por lo tanto se comportan por norma general de forma parecida.

Bien es cierto que con un conjunto tan pequeño de datos es lógico que tomen valores similares tanto en el conjunto de validación como el de test, aunque sigue resultado sorprendente que se haya llegado a un punto de igualdad en ambas decisiones realizadas por ambos modelos.

Por otro lado, también resulta impactante obtener los mismos resultados tanto en el modelo mejorado de la red neuronal como en el simple, pese a haber jugado con los valores _size_ y _decay_. Esto implica que o bien el conjunto no es lo suficientemente grande para que haya un cambio significativo, o bien hay que ser más precisos con los valores escogidos, ya que obtener la misma puntuación es un caso que aparentemente consideraría aislado.

## Tabla de resultados

| Modelo              | Precisión | AUC  | Score Kaggle |
|---------------------|-----------|------|--------------|
| modelo_rf1          | 0.7932331 | 0.81 | No enviado   |
| modelo_rf2          | 0.8305085 | 0.86 | No enviado   |
| modelo_rna          | 0.8305085 | 0.81 | 0.77272      |
| modelo_rna_mejorado | 0.8305085 | 0.86 | 0.77272      |